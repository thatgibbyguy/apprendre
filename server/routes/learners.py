...
from server.models.database import get_connection, get_learner
    ...
    try:
        ...
        if active_row:
            ...
            try:
                ...
                transcript = []
            # Count only user+assistant turns (not system)
            exchange_count = sum(
...
async def get_learner_profile(learner_id: int) -> dict:
    """Return a learner's profile by ID."""
    conn = get_connection()
    try:
        learner = get_learner(conn, learner_id)
        if learner is None:
...
@router.post("/")
async def create_learner_endpoint(body: CreateLearnerBody) -> dict:
    """Create a new learner and return their ID."""
    from server.models.database import create_learner
    ...
    try:
        learner_id = create_learner(
            conn,
            body.name,
            level_listening=body.level_listening,
            level_reading=body.level_reading,
            level_speaking=body.level_speaking,
            level_writing=body.level_writing,
